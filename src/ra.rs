...
impl Session {
    ...
    pub fn lock(
        ...
        for ((k, _), revnum_val) in path_revs.iter().zip(revnum_values.iter()) {
            unsafe { hash.insert(k.as_bytes(), revnum_val as *const _ as *mut std::ffi::c_void); }
        }
    ...
    pub fn unlock(
        ...
        for (k, v) in path_token_ptrs.iter() {
            unsafe { hash.insert(k.as_bytes(), *v as *mut std::ffi::c_void); }
        }
        let err = unsafe {
...
mod tests {
    ...
    fn create_test_repo_with_session() -> (tempfile::TempDir, crate::repos::Repos, Session) {
        ...
        auth_baton.set(crate::auth::AuthSetting::Username("testuser")).unwrap();
