...
fn validate_path_string(path_str: &str) -> Result<(), crate::Error> {
    ...
    if path_str.contains("\x7f") || path_str.contains("\r") || path_str.contains("\n") {
        return Err(crate::Error::from_str("Path contains invalid control characters"));
    }
...
pub fn canonicalize_dirent(path: &std::path::Path) -> Result<std::path::PathBuf, crate::Error> {
    with_tmp_pool(|pool| unsafe {
        let path_str = path.to_str().ok_or_else(|| crate::Error::from_str("Invalid path encoding"))?;
...
pub fn join_dirents(base: &Dirent, component: impl AsRef<std::path::Path>) -> Result<Dirent, crate::Error> {
    with_tmp_pool(|pool| unsafe {
...
mod tests {
    ...
    fn test_is_child() {
        ...
        assert_eq!(result.as_ref().map(|d| d.as_path()), Some(std::path::Path::new("project")));
    ...
    fn test_skip_ancestor() {
        ...
        assert_eq!(result.as_ref().map(|d| d.as_path()), Some(std::path::Path::new("project/file.txt")));
        ...
        let path = Dirent::new("/home/user").unwrap();
        let result = skip_ancestor(&ancestor, &path).unwrap();
        assert_eq!(result.as_ref().map(|d| d.as_path()), Some(std::path::Path::new("")));
    }
        ...
        for path in test_cases {
            ...
            if let (Ok(basename), Ok(dirname)) = (dirent.basename(), dirent.dirname()) {
                if let Ok((split_dirname, split_basename)) = dirent.split() {
                    assert_eq!(basename.as_str(), split_basename.as_str(), 
                              "basename() and split() basename should match for path: {}", path);
                    assert_eq!(dirname.as_str(), split_dirname.as_str(),
                              "dirname() and split() dirname should match for path: {}", path);
                }
            }
            
            // Test that join operations are safe
            let joined = dirent.join("test").unwrap();
            assert!(joined.as_str().contains("test"), "Join should include component");
        }
    }
}