    ...
    F: FnMut(&[u8], *mut std::ffi::c_void) -> IterResult<()>,
{
    with_tmp_pool(|pool| {
        ...
        let mut callback_wrapper = |key: &[u8], value: *mut std::ffi::c_void| -> IterResult<()> {
            callback(key, value)
        };
        ...
        extern "C" fn c_callback(
            ...
            let key_slice = unsafe {
                std::slice::from_raw_parts(key as *const u8, klen as usize)
            };
    ...
    F: FnMut(*mut std::ffi::c_void) -> IterResult<()>,
{
    with_tmp_pool(|pool| {
        let mut completed: subversion_sys::svn_boolean_t = 0;
        let mut callback_wrapper = |item: *mut std::ffi::c_void| -> IterResult<()> {
            callback(item)
        };
...
mod tests {
    ...
    fn test_array_iteration() {
        ...
        let completed = array.iter_items(|item| {
